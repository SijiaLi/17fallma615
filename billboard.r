...
# check row 288
# similar function: gsub
# compare with: tmp <- raw$track[288]; str_replace(tmp, " (.*?)", "")
...
# compare with: unique(clean[c("artist", "track", "time")])
song$id <- 1:nrow(song)
...
# a chunck of code used for text analysis data cleaning with gsub
# x here is the document
x <- iconv(x, "ASCII", "UTF-8", sub="")    # remove all emojis
x <- gsub("@\\w+ *", "", x)    # remove @
x <- gsub("#\\w+ *", "", x)    # remove hashtag
x <- gsub("\n", " ", x)    # remove paragraphs
x <- gsub("[^a-zA-Z #]","",x)    # save english letters only
x <- gsub("https\\w+ *", "", x)    # remove urls
x <- tolower(x)    # to lower letters

# example with regular expression: check help for more
?regex
test <- c("15: .0234 , 12: .0151")
gsub("^15","x",test)
gsub("^([0-9]+)","x",test)
# here [0-9]+ means "any number"
# ^ means "at the start of the string"
gsub("([0-9]+):","x:",test)
# here ([0-9]+): means "any number followed by a colon
